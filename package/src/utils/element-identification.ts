...
export function getElementPath(target: HTMLElement, maxDepth = 4): string {
  ...
  while (current && depth < maxDepth) {
    ...
    // Append first (and last, if >1) data-* attribute
    const dataAttrs = Array.from(current.attributes).filter((a) => a.name.startsWith('data-'))
    if (dataAttrs.length > 0) {
      identifier += `[${dataAttrs[0].name}="${dataAttrs[0].value}"]`
      if (dataAttrs.length > 1) {
        const last = dataAttrs[dataAttrs.length - 1]
        identifier += `[${last.name}="${last.value}"]`
      }
    }
